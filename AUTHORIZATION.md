...
The packaged Shopee RPA requires online authorization before it runs.
...
```text
shopee_creator_outreach_rpa
```

## Upgrade Existing Computers

Keep each computer's existing `config\automation_config.json`, `images\`, `data\`, OCR regions, coordinates, and templates. Upgrade by replacing only the exe and adding an `auth_config.json` with that user's license key.
